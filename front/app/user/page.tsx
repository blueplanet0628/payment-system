...
const Billing: React.FC<BillingProps> = ({ user }) => {
  ...
  useEffect(() => {
    if (extraSheets > 0) {
      setLoading(true);
      axios
        .post("http://192.168.131.55:3000/api/create-payment-intent", { extraSheets })
        .then((response) => {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://192.168.131.55:3000/api/update-sheets", {
          email: user.email,
          extraSheets,
        });
...
const BillingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://192.168.131.55:3000/google-sheet")
  ...
  const InstagramTable: React.FC<Props> = ({ user }) => {
    ...
    useEffect(() => {
      const fetchInstagramIds = async () => {
        ...
        try {
          const res = await axios.get(`http://192.168.131.55:3000/api/instagram-id/${user.sheetId}`);
    ...
    const handleSave = async () => {
      ...
      try {
        const response = await axios.post('http://192.168.131.55:3000/api/save-instagram-id', {
          id: newId,
          sheet: user.sheetId,
        });
    ...
    const handleEditSave = async () => {
      ...
      try {
        const response = await axios.post('http://192.168.131.55:3000/api/edit-instagram-id', {
          oldId: editId,
          newId: newId,
          sheet: user.sheetId,
        });
    ...
    const del = async (instagramId: string) => {
      try {
        const response = await axios.post('http://192.168.131.55:3000/api/delete-instagram-id', {
          id: instagramId,
          sheet: user.sheetId,
        });
